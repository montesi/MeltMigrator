...
% % load comsol model
tic;
Model=mphload('MeltMigrationDemo_01.mph'); % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MODEL DEPENDENT
disp(sprintf('>>> COMSOL model loaded in %g s',toc));
NameTag='DEMO'; % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MODEL DEPENDENT

% % key parameters
SpreadingRate_Plate=2.0; % [cm/yr] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MODEL DEPENDENT
SpreadingRate_etalP=2.0; % [cm/yr] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MODEL DEPENDENT
...
TrimDistance=20; % distance to trim close to boundary [km] <<<<<<<<<<<<<<<< MODEL DEPENDENT

Switch_asymmetryStudy=0; % switch for asymmetric melting study
...
SaddleInitialPoint=[250,400;325,225;375,175]; % starting point for saddle point search <<<< MODEL DEPENDENT
...
ExtractionWidth=[4]; % extraction width
ExtractionDepth=[20]; % extraction depth
...
Geometry.ModelBoundary.x=[0,600]; 
Geometry.ModelBoundary.y=[0,550]; 
Geometry.ModelBoundary.z=[0,100]; 
% % plate boundary geometry <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MODEL DEPENDENT
Geometry.PlateBoundary.x=[200,200,300,300,400,400]; 
Geometry.PlateBoundary.y=[550,400,400,250,150,0]; 
Geometry.PlateBoundaryType=[1,2,1,3,1]; % 1: ridge, 2: transform, 3: oblique segment
...
Res.MeltTrajectory=5; % melt line resolution
